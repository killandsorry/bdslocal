...
if($rcat    = $db_cat->fetch()){
    if($rcat['cat_parent_id'] == 0){
        $sql_where  = " AND pro_cat_id IN (". $rcat['cat_child_id'] .")";
    }else{
        $sql_where  = " AND pro_cat_id = " . intval($rcat['cat_id']);
    }
}else{
    header_301(PATH_ROOT);
    exit();
...
$db_count		= new db_count("SELECT COUNT(*) AS count FROM products
									WHERE 1 ". $sql_where ." AND pro_active = 1");
...
<div id="main_container">
    <?
    include '../includes/inc_products_list.php';
    ?>
</div>
<div id="footer">
    <?
