...
class user{
   ...
   protected $use_right;
   protected $use_right_1;
   protected $use_right_2;
   protected $use_supplier = 0;
	/*
	init class
	...
	*/
	function __construct($login_name="", $password="", $tcookie = 0){
		...
		$db_user = new db_query("SELECT *
			...
			if (($password==$row["use_password"] || $password== md5($this->key_sercurity_fake . $row["use_password"]) || $password== md5($this->key_sercurity_dev . $row["use_password"])) && $row["use_active"]==1) {

				//kiem tra xem neu la fake login thi gan vao bien de con check
				if($password== md5($this->key_sercurity_fake . $row["use_password"])){
					$this->isFakeLogin = true;
				}

				//kiem tra xem neu la dev login thi gan vao bien de con check
				if($password== md5($this->key_sercurity_dev . $row["use_password"])){
            ...
            $this->use_right_1			= intval($row["use_right_1"]);
            $this->use_right_2			= intval($row["use_right_2"]);
            $this->use_supplier		= intval($row["use_supplier"]);
				$this->useField  			= $row;
			}
		}
		unset($db_user);
