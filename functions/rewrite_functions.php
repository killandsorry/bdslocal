...
function gen_link_detail($data = array()){
    $link   = '';
    $id = isset($data['pro_id'])? intval($data['pro_id']) : 0;
    $name   = isset($data['pro_name'])? ($data['pro_name']) : '';
    $address   = isset($data['pro_address'])? ($data['pro_address']) : '';
    if($id > 0 && $name != ''){
        $link   = '/' . removeTitle($name . ' ' . $address) . '-p' . $id;
    }
    return $link;
}


function gen_link_news_detail($data = array()){
    $link   = '';
    $id = isset($data['new_id'])? intval($data['new_id']) : 0;
    $name   = isset($data['new_name'])? ($data['new_name']) : '';
    if($id > 0 && $name != ''){
        $link   = '/news/' . removeTitle($name) . '-n' . $id;
    }
    return $link;
}


function gen_link_category($data = array()){
    $link   = '';
    $id = isset($data['cat_id'])? intval($data['cat_id']) : 0;
    $name   = isset($data['cat_name'])? ($data['cat_name']) : '';
    $type   = isset($data['cat_type'])? ($data['cat_type']) : '';

    $prefix = ($type == 'products')? 'cp' : 'cn';
    if($id > 0 && $name != ''){
        $link   = '/' . removeTitle($name) . '-'. $prefix . $id;
    }
    return $link;
}
...
function createLinkBlogCategory($row = array()){
   $id   = isset($row['cat_id'])? intval($row['cat_id']) : 0;
   $blo_title   = isset($row['cat_name'])? ($row['cat_name']) : '';
...
function create_link_help($data = array()){
   $id = isset($data['hel_id'])? intval($data['hel_id']) : 0;
   $name = isset($data['hel_title'])? removeTitle($data['hel_title']) : '';
   $link = '/' . $name . '-help' . $id;
   return $link;
}