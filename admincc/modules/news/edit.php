...
//$html_clean			= new html_cleanup($nec_content);
//$html_clean->clean();
//$nec_content	= $html_clean->output_html;
   ...
   $errorMsg .= $myform->checkdata();
	...
	if($errorMsg == ""){
		...
		if($errorMsg == ''){
			...
			$db_ex	= new db_execute($mydes->generate_update_SQL('nec_id', $record_id));
			unset($db_ex);
			redirect($listing);
		}
	}
...
$query = "SELECT * FROM " . $fs_table . "  INNER JOIN news_content  ON nec_id = new_id WHERE " . $field_id . " = " . $record_id;
//echo $query;
