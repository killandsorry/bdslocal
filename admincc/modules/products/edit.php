...
$myform->add("pro_name", "pro_name", 0, 0, "", 1, "Chưa nhập tên dự án", 0, ""); //tiêu đề tin
...
if($action == "execute"){
    ...
    $prd_furniture_logia_room =  getValue('prd_furniture_logia_room', 'str', 'POST', '');
    //echo $prd_furniture_logia_room;
    //die();
    ...
    if(isset($_FILES['prd_position_img'])) {
    ...
    if(isset($_FILES['prd_utilities_in_img'])) {
    ...
    if(isset($_FILES['prd_utilities_out_img'])) {
	...
	if($fs_errorMsg == ""){
		...
		if($record_id > 0){
		    //echo $myformdes->generate_update_SQL('prd_id', $record_id);
            $db_detail = new db_execute($myformdes->generate_update_SQL('prd_id', $record_id));
...
<p align="center" style="padding-left:10px;">
    ...
    <?=$form->text('Địa chỉ', 'pro_address', 'pro_address', $pro_address, 'Địa chỉ', 1, 500, '', 255)?>
    <?=$form->text('Title', 'pro_title', 'pro_title', $pro_title, 'Title', 1, 500, '', 255)?>
    ...
    <tr>
        ...
        <td class="form_text"  id="load_district">
            <?
            if($pro_cit_id > 0){
                echo '<select id="pro_disctrict_id" name="pro_disctrict_id" class="form_control" >';
                $db_cit = new db_query("SELECT * FROM city WHERE cit_parent_id = " . $pro_cit_id);
                while ($rci = $db_cit->fetch()){
                    echo '<option value="'. $rci['cit_id'] .'" '. (($rci['cit_id'] == $pro_district_id)? 'selected' : '') .'>'. $rci['cit_name'] .'</option>';
                }
                echo '</select>';
            }
            ?>
