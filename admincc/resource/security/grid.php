	...
	*/
	...
	function start_tr($i, $record_id, $add_html = ""){
		...
		$str = '<tbody id="tr_' . $record_id . '">
					<tr ' . (($i%2==0) ? 'bgcolor="#f7f7f7"' : '') . $add_html . '>';
		$str .= $this->showid($i, $page);
	...
	function addSearch($name,$field,$type,$value = '',$default=""){
		...
		switch($type){
			...
			case "text":
				...
				$str .= '<input type="text"  class="textbox" name="' . $field . '" id="' . $field . '" style="width:130px;"  onfocus="if(this.value==\'' . $default . '\') this.value=\'\'" onblur="if(this.value==\'\') this.value=\'' . $default  . '\'" value="' . $value . '">';
			break;
		}
